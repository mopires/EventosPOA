...
namespace EventosPOA
{
    class MySQL
    {
        ...
        {
            conexao = new MySqlConnection("server="+server+";user id="+user+";" +
        ...
        public string CadastroUsuario(Usuario user)
        {
            ...
            comando = new MySqlCommand("insert into usuario " +
                                       "(username, pass, nome, sobrenome, dataNascimento) " +
                                       "values('" + user.GetUsername() +"','"
                                       + user.GetPass() +           "','"
                                       + user.GetName() +           "','" 
                                       + user.GetSobrenome() +      "','"
                                       + user.GetDataNascimento().Date + "'"
        ...
        {
            string query = "SELECT username, pass, nome, sobrenome, dataNascimento FROM usuario WHERE " +
                           "username = '" + username + "' " +
                           "AND pass = '" + pass + "'";
                ...
                if (GetHasRows(data))
                {
                    ...
                    try
                    {
                        Usuario user = new Usuario(data.GetString(0),
                                               ...
                                               data.GetString(3),
                                               data.GetDateTime(4)
                                               ); //erro na senha data.GetString(2)
                        conexao.Close();
                        return user;
                    }
        ...
        {
            string format = "yyyy-MM-dd HH:mm"; //alterar no banco o formato
